...
#######For this, we have electrs node. We have the lightning node. Should we create a light pruned node that we can  
...
# Install/Setup/Enable SSH(D) !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! We need users that connect to the RPC of bitcoin. 
...
# Generate Wallets !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! We are going to use this to load satoshi Coins and to redeam cold storage
sudo -u bitcoin /usr/bin/bitcoin-cli -micro -datadir=/var/lib/bitcoin -conf=/etc/bitcoin.conf --named createwallet wallet_name="import" descriptors=false passphrase=$(sudo cat /root/passphrase) load_on_startup=true
...
sudo -u bitcoin /usr/bin/bitcoin-cli -micro -datadir=/var/lib/bitcoin -conf=/etc/bitcoin.conf --named createwallet wallet_name="mining" passphrase=$(sudo cat /root/passphrase) load_on_startup=true
    ############# Ok, we still need secure communication. Otherwise, it is left exposed on the network. We want a tight run ship; therefore, we can have multiple ##################
...
bash ~/microbank/scripts/teller.sh -i
