...
This readme was generated by the "btc-explorer.sh", a "Bitcoin Blockchain Explorer" install script.
    ...
    Running Information: sudo systemctl status btc-rpc-explorer
...
sudo -u btcexp btc-rpc-explorer

# NOTES for the TOP Configure NGINX Reverse Proxy to point to the elctrum server with configured subdomain?????????
---http://127.0.0.1:3002

############ Variables ############## #### Microcurrency will need the password too unless we make it fixed !!!!!!!!!!!!!!!!!!!!
52:54:00:3b:d9:4f   192.168.2.5     btc-electrum    Bitcoin Electrum Server
52:54:00:ca:05:63   192.168.2.12    btc-node    Full Indexed Bitcoin Node
