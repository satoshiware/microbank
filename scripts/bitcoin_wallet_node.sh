...
Use this node to manage the following wallets:
    ...
    The "btcpay" wallet is used to receive on-chain payments via the btcpay server node
...
Note: This is a pruned bitcoin node. The rescan feature is not possible on a pruned blockchain; therefore,
    ...
    node synchronization process (by copying blockchain data from elsewhere) as it may skip over wallet transactions.
    THE LAST btcpay WALLET BACKUP MAY NOT ACCOUNT FOR SUFFICIENT GAP LIMIT, before synchronization advances too much
    execute the following commands to increase the btcpay wallet keypool to account for all of its txs (\$FINALKEYCOUNT)!
        btc -rpcwallet=btcpay walletpassphrase \$(sudo cat /root/passphrase) 60
        btc -rpcwallet=btcpay keypoolrefill \$FINALKEYCOUNT
...
sudo -u bitcoin /usr/bin/bitcoin-cli -datadir=/var/lib/bitcoin -conf=/etc/bitcoin.conf --named createwallet wallet_name="btcpay" passphrase=$(sudo cat /root/passphrase) load_on_startup=true

# Increse BTCPAY wallet gap limit to 25000
sudo -u bitcoin /usr/bin/bitcoin-cli -datadir=/var/lib/bitcoin -conf=/etc/bitcoin.conf -rpcwallet=btcpay walletpassphrase $(sudo cat /root/passphrase) 600
sudo -u bitcoin /usr/bin/bitcoin-cli -rpcwallet=btcpay -datadir=/var/lib/bitcoin -conf=/etc/bitcoin.conf keypoolrefill 25000
...
sudo ln -s /var/lib/bitcoin/btcpay ~/backup
...
if [[ -d ~/restore ]]; then
    ...
    sudo chown -R bitcoin:bitcoin ~/restore/btcpay
    ...
    sudo rm -rf /var/lib/bitcoin/btcpay
    ...
    sudo mv -f ~/restore/btcpay /var/lib/bitcoin
