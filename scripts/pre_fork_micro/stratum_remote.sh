...
The "stratum_remote.sh" script create an remote mining access point to stratum node (within a cluster).
...
To execute this script, login as a sudo user (that is not root) and execute the following commands:
    ...
    bash ./microbank/scripts/pre_fork_micro/stratum_remote.sh
    rm -rf microbank
