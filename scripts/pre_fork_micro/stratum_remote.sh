...
This readme was generated by the "stratum_remote.sh" install script.
This script creates a remote mining access point to stratum node (within a cluster).
...
# Generate public/private keys if it does not exist
if ! sudo test -f "/root/.ssh/p2pkey"; then
...
ExecStart=/usr/bin/autossh -M 0 -NT -o ServerAliveInterval=30 -o ExitOnForwardFailure=yes -o "ServerAliveCountMax 3" -i /root/.ssh/p2pkey -L 0.0.0.0:3333:localhost:3333 -p \${TARGET_PORT} stratum@\${TARGET}
...
if [[ "${HOSTSIG}" == *"${HOSTKEY}"* ]]; then
    ...
    echo "${HOSTSIG} # Remote Mining Connection, ${TARGETADDRESS}:${SSHPORT}" | sudo tee -a /root/.ssh/known_hosts
else
