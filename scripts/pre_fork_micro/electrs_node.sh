...
It installs both a micro node and an micro electrum server (with electrs Blockstrem fork).

A micro node has been installed and readied to receive connections from the electrs daemon and external services.

The electrs server indexes the entire micro blockchain, and the resulting index enables fast queries for any given user wallet,
...
FYI:
    ...
    The Bitcoin micro configuratijon: /etc/bitcoin.conf
...
sudo ufw allow 13000 # Open up Electrs HTTP server
sudo ufw allow 51001 # Open Electrs JSON RPC connection access
