...
This readme was generated by the "p2p_node.sh" install script.
The "p2p_node.sh" script installs a "p2p" micronode (within a cluster) which is used to manage connections with all other internal and external nodes.
...
To execute this script, login as a sudo user (that is not root) and execute the following commands:
    ...
    bash ./microbank/scripts/pre_fork_micro/p2p_node.sh
    rm -rf microbank
