...
The "micronode_cluster_wallet.sh" script installs a "wallet" micronode (within a cluster) which is used to manage the microcurrency hotwallet (including payouts).
...
To execute this script, login as a sudo user (that is not root) and execute the following commands:
    ...
    sudo chmod +x micronode_cluster_wallet.sh
    ./micronode_cluster_wallet.sh
