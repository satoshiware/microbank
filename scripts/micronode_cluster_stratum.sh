...
### TODOS:
    # Finish pertinent information
    # (DONE)Setup cron job to call stmutility to change the address if need be. (a few times an hour)

    ##bash ~/micronode/scripts/mnconnect.sh -i
    ##bash ~/micronode/scripts/stmutility.sh -i
...
/var/log/ckpool/ckpool.log!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Log file location
Update/Upgrade "wallet" micronode utilities
    ...
    rm -rf microbank
        ...
        Notes:
        Point the miner to this node or the "Remote Access Point" (with an internal static ip) to port 3333
            Example: stratum+tcp://$IP_ADDRESS:3333
        Use the "mnconnect" utility to view or delete remote connections
        Use the "mnconnect" utility to see or generate all the connection info' for this node
        Run the "micronode_stratum_remote.sh" on a SBC (e.g. Raspberry Pi Zero 2) to setup a "Remote Access Point"
...
sudo mv ~/micronode/python/rpcauth.py /usr/share/python/rpcauth.py
