...
SQ3DBNAME=/var/lib/btcofaz.db.development # Uncomment to work on the development database.
if [[ $SQ3DBNAME == *"development"* ]]; then
	echo "Did you make a recent copy of the production database for development mode?"
	echo "\"sudo cp /var/lib/btcofaz.db /var/lib/btcofaz.db.development\""
	echo "Did you make backups of the production database?"
	echo "\"sudo cp /var/lib/btcofaz.db /var/lib/btcofaz.db.bak(1, 2, 3...)\""
	read -p "You are in development mode! Press any enter to continue ..."
fi

###### Logging and Email system are up next.
...
# Email
#### 1) if sats are too low in market data, bitcoin value will show 0. What can be done?
#### 3) Can't wait to link to an explorerer and view address and transaction and network information. How to get those links to work?
#### 5) warning about loosing card and end of contract
...
#### sudo sqlite3 $SQ3DBNAME "DELETE FROM txs WHERE txid is NULL;"
# sudo sqlite3 $SQ3DBNAME "DELETE FROM txs WHERE tx_id = (SELECT MAX(tx_id) FROM txs);" # Remove last tx
...
# sudo sqlite3 $SQ3DBNAME ".mode columns" "SELECT * FROM payouts WHERE epoch_period = (SELECT MAX(epoch_period) FROM payouts);" # The latest epoch added to the DB
...
if [[ $1 = "-h" || $1 = "--help" ]]; then # Show all possible paramters
	...
	Options:
	  ...
	  -f, --fees		Calculate total fees for the latest Difficulty epoch?????????????????????????????????????????????????????????????????????????
...
EOF
	...
	if [ ! -f /usr/local/sbin/payouts ]; then
		sudo cat $0 | sed '/Install this script (payouts)/d' | sudo tee /usr/local/sbin/payouts > /dev/null
...
elif [[ $1 = "-f" || $1 = "--fee" ]]; then # Calculate total fees for the latest Difficulty epoch?????????????????????????????????????????????????????????????????????????
...
EOF
	...
	# Format the array data togethor and dump to log.............. BETTER COMMENT
	for ((i=0; i<${#notify_data[@]}; i++)); do
		if ! [[ -z ${notify_data[i]} || -z $SATRATE || -z $USDSATS || -z ${addresses[i]} || -z ${txids[i]} ]]; then
